...
(function($, window, document, undefined) {
  $.fn.Newsticker = function(options) {
    ...
    function create() {
      ...
      self.addClass('newsticker').html('<div class="newsticker-container"><div class="newsticker-one"></div><div class="newsticker-two"></div></div>');

      container = self.find('.newsticker-container');
      one = self.find('.newsticker-one');
      two = self.find('.newsticker-two');
      both = self.find('.newsticker-one, .newsticker-two');
    ...
    function start() {
      ...
      if (settings.type === 'static') {
        ...
        container.prepend('<div class="newsticker-loader"></div>');
        $.when(ajax(settings.source, settings.dataType, settings.callback)).done(function() {

          container.find('.newsticker-loader').fadeOut(300, function() {
            run(content, 0);
            $(this).remove();
          });
